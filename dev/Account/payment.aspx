<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="payment.aspx.vb" Inherits="WebApplication1.order" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#cboEmployees").change(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "payment.aspx/GetTaxRates",
                    data: "{'jurID': '" + record_id + "'}",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (returnVal) {
                        //var num = 0.01
                        var num = $("#cboEmployees").val() * 0.01;
                        var GST;
                        var QST;
                        var HST;

                        GST = num * returnVal.d[0];
                        $("#txtGST").val(GST.toFixed(2));
                        HST = num * returnVal.d[1];
                        $("#txtHST").val(HST.toFixed(2));
                        QST = num * returnVal.d[2];
                        $("#txtQST").val(QST.toFixed(2));

                        $("#txtSubTotal").val(num.toFixed(2));
                        num = $("#cboEmployees").val() * 0.01 + (GST * 1) + (HST * 1) + (QST * 1);
                        $("#txtTotal").val(num.toFixed(2));

                        if ($("#txtGST").val() == 0) { $("tr:#Row-GST").hide(); } else { $("tr:#Row-GST").show(); }
                        if ($("#txtHST").val() == 0) { $("tr:#Row-HST").hide(); } else { $("tr:#Row-HST").show(); }
                        if ($("#txtQST").val() == 0) { $("tr:#Row-QST").hide(); } else { $("tr:#Row-QST").show(); }
...
<body style=" background-image:url('../images/bg2.png'); background-position:top; background-repeat:repeat-x;">
    ...
    <table border="0" style="height: 325px; background-image:url(../images/bg2.jpg);" width="99%">
            ...
            <table border="0">
                ...
                <tr><td rowspan="10" valign="top"><img src="images/payment.png" width="200px" alt="" /></td><td></td><td></td><td rowspan="10" colspan="10" valign="top" style="color:#cd1e1e;"><table><tr><td width="15px" valign="middle"><img src="images/redbullet.png" /></td><td valign="middle">Taxes will be based on the jurisdiction selected in previous screen.</td></tr><tr style="height:20px;"><td></td></tr><tr><td  valign="middle"><img src="images/redbullet.png" alt="" /></td><td>Please contact Advataxes if you have more than 99 employees.</td></tr><tr style="height:20px;"><td></td></tr><tr><td><img src="images/redbullet.png" alt="" /></td><td valign="top">You will be redirected to Paypal when you click "Subscribe" to complete the transaction. </td></tr></table></td></tr>
